...
### Bridge the DO into a typed RpcClient
...
The `url` is a placeholder — the request never hits the network; it
short-circuits to `tasksDO.getByName(id).fetch`.

### Wire the proxy handlers

The Worker's `*DO` handlers just construct a per-request DO client
and forward the typed call:
...
See the [RPC Worker tutorial](/tutorial/cloudflare/rpc-worker) for an
...
`counters.getByName(id)` is a typed `RpcClient<CounterRpcs>` rather
than alchemy's built-in DO method bridge. Reach for this whenever DO
method return values cross a `Schema.Class` boundary — the built-in
bridge `JSON.stringify`s each value and loses class identity, while
the RPC namespace round-trips through the shared `RpcSerialization`
codec. See the [RPC Durable Object tutorial](/tutorial/cloudflare/rpc-durable-object)
